...
interface SectionBreakerProps {
  to: string
  icon: React.ReactNode        // small icon for the badge
  watermarkIcon: React.ReactNode // large icon used as background watermark
}
...
export default function SectionBreaker({ to, icon, watermarkIcon }: SectionBreakerProps) {
  ...
  return (
    <div
      ref={ref}
      className="w-full bg-[#eceef3] relative overflow-hidden flex flex-col items-center justify-center gap-4"
      style={{ minHeight: 200, paddingTop: 32, paddingBottom: 32, userSelect: 'none' }}
    >
      ...
      <div
        style={{
          ...
          color: 'rgba(76,175,114,0.07)',
          pointerEvents: 'none',
          opacity: vis ? 1 : 0,
          transition: 'opacity 0.8s ease 0.1s',
        }}
      >
        {/* Clone the watermarkIcon but override size via CSS */}
      ...
      >
        ...
        <div style={{ flex: 1, height: 1.5, position: 'relative', overflow: 'hidden', borderRadius: 2 }}>
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to right, transparent, rgba(76,175,114,0.22))' }} />
          <div style={{ position: 'absolute', top: 0, bottom: 0, width: '40%', background: 'linear-gradient(to right, transparent, #4caf72, transparent)', animation: 'railLeft 2s ease-in-out infinite' }} />
        </div>

        {/* Centre badge — icon + pulse dot only, no number */}
        <div style={{
          ...
          <div style={{
            ...
            background: '#eceef3',
            boxShadow: 'inset 2px 2px 6px #d1d3da, inset -2px -2px 6px #ffffff',
        ...
        {/* Right rail */}
        <div style={{ flex: 1, height: 1.5, position: 'relative', overflow: 'hidden', borderRadius: 2 }}>
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to left, transparent, rgba(76,175,114,0.22))' }} />
          <div style={{ position: 'absolute', top: 0, bottom: 0, width: '40%', background: 'linear-gradient(to left, transparent, #4caf72, transparent)', animation: 'railRight 2s ease-in-out infinite' }} />
        </div>
      </div>
      ...
      <div style={{
        ...
        height: 3,
        borderRadius: 2,
        background: 'linear-gradient(to right, transparent, #4caf72, transparent)',
